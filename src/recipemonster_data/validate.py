...
def validate_catalog(directory: Path) -> dict[str, int]:
    ...
    if not nutrition_rows:
        raise ValueError("catalog has no nutrition records")
    ingredient_ids = validate_nutrition_rows(nutrition_rows)
    result = {"ingredients": len(nutrition_rows)}
    language_ids: dict[str, set[str]] = {}
    for language in SUPPORTED_LANGUAGES:
        rows = read_csv(directory / f"ingredients_{language}.csv", NAME_COLUMNS)
        language_ids[language] = validate_name_rows(rows, language, ingredient_ids)
        result[f"names_{language}"] = len(rows)
    for language in SUPPORTED_LANGUAGES:
        missing = ingredient_ids - language_ids[language]
        if missing:
            raise ValueError(f"catalog is missing {language} names for {len(missing)} ingredients")
    return result
...
def validate_nutrition_rows(rows: list[dict[str, str]]) -> set[str]:
...
def validate_name_rows(
    ...
    for row in rows:
        ...
        if ingredient_id not in ingredient_ids:
            raise ValueError(f"{language} name references unknown ingredient: {ingredient_id}")
